...
### 2027年度カリキュラム改訂方針（2026-04-25 メモ）

- U00（コンピュータ基礎）から **A・Bフェイズを削除** し，Cフェイズ（webexercises自動採点）のみ残す
- 理由: ターミナル課題（0-B-1, 0-B-2, 0-B-6, および 0-A-2 のフォルダ構造作成）が3年生にとって最初の壁になっており，Rチュートリアル全体への着手が遅れる原因になっている
- Cフェイズはブラウザで回答できるので負担小。知識のチェックとして十分機能する
- 事前知識セクション（パス・文字コード・ファイル名規則等）は卒論進行時に必須の知識なので削除しない
- 今年度（2026年度）はそのまま運用。改訂は2027年度4月から
- 関連: 来年度の3年生ガイダンス資料（`zemi-template/`等）も合わせて更新する必要あり
