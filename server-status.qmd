...
async function loadServerStatus() {
  try {
    // データは data/server_status.json（Quarto の render 対象外）に置き、
    // raw.githubusercontent から直接取得する。これで docs/ の再ビルドと完全に独立し、
    // サイト再 render で JSON が消える問題が起きない。
    const response = await fetch('https://raw.githubusercontent.com/kosugitti/kosugi-labo/main/data/server_status.json?' + Date.now());
...
function renderDashboard(data) {
  ...
  servers.forEach(server => {
    ...
    // 死活だけ見ているマシン（ping応答のみ・CPU等は取らない）
    const aliveOnly = server.alive_only === true;
    ...
    const thirdMetric = hasBackup
      ...
      : `<div class="metric">
            ...
            <div class="metric-label">${(server.temp_kind === 'cpu' || server.name === 'Raspberry Pi') ? 'CPU温度' : 'GPU温度'}</div>
          </div>`;
    ...
    html += `
      <div class="server-card ${isOnline ? '' : 'offline'}">
        ...
        ${aliveOnly ? '<div class="alive-note">死活監視のみ</div>' : isOnline ? `
