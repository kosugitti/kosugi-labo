...
エラーが出ていない時は，下のようなダイアログが一瞬出てすぐ消えます。
赤字や `error:` `CONFLICT` `[rejected]` が含まれていなければ成功です。
特に Pull で `Already up to date.` だけが出ると「これってエラー?」と
不安になりがちですが，「リモートに新しいものが何もなかった」という意味の正常表示です。

### Pull 成功 (リモートに更新があった場合)

![Pull 成功 (Fast-forward) ダイアログ](img/git-success-pull.png)

`Fast-forward` + `Updating <sha>..<sha>` と表示されれば，リモートの変更がきれいに取り込まれています。

### Pull 成功 (リモートに何もなかった場合)

![Pull 成功 (Already up to date) ダイアログ](img/git-success-uptodate.png)

`Already up to date.` はエラーではなく，「すでに最新です」という意味の正常表示です。

### Push 成功

![Push 成功ダイアログ](img/git-success-push.png)
...
![Pull 時のコンフリクト画面 (RStudio)](img/git-trouble-conflict.png)
...
![Push 拒否ダイアログ (RStudio)](img/git-trouble-rejected.png)
...
![Pull 拒否 (local changes overwrite) ダイアログ (RStudio)](img/git-trouble-overwrite.png)
